...
const paymentDetail = joi.object().keys({
    ...
    cardName: joi.string().required().regex(pattern.strPattern).max(20).min(2),
    cardHolderName: joi.string().required().regex(pattern.strPattern).max(30),
...
const updatePayment = joi.object().keys({
    ...
    cardName: joi.string().regex(pattern.strPattern).min(2).max(20),
    cardHolderName: joi.string().regex(pattern.strPattern).max(30),
    cvvNo: joi.string().max(3).min(3).regex(pattern.cvvPattern),
    expDate: Joid.date().format("MM/YY").greater('now'),
})
